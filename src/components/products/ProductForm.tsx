...
export default function ProductForm({
  ...
  const onSubmit = async (data: ProductFormSchema) => {
    ...
    // If creating, require files. If editing, files might already exist.
    if (mode === "create") {
      if (!thumbFile) {
    ...
    try {
      if (thumbFile) {
        ...
        if (thumbResult) thumbnailUrl = thumbResult.secureUrl;
      }

      if (productFile) {
        const fileResult = await uploadFile(productFile);
        if (fileResult) fileUrl = fileResult.secureUrl;
      }
      ...
      if (mode === "create") {
        ...
        const res = await ProductAPI.updateById(initialData._id, updatePayload);
  ...
  const displayThumbnail = thumbPreview || initialData?.thumbnailUrl;
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between glass p-4 rounded-2xl border-card-border/60 shadow-sm gap-4">
            ...
            <div className="flex items-center gap-3 self-end sm:self-auto">
              ...
              <Button
                variant="primary"
                onClick={() => {
                  if (mode === "create") setValue("status", "published");
                }}
                className="px-4 py-2"
                type="submit"
                isLoading={isUploading}
        ...
        <div className="space-y-8 mt-8">
              ...
              {displayThumbnail ? (
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    PNG, JPG, or WEBP (max.{" "}
                    {UPLOAD_CONFIG.THUMBNAIL.MAX_SIZE_MB}MB)
                  </p>
                </div>
              )}
              {thumbError && (
          ...
          <section className="glass rounded-2xl p-6 md:p-8 space-y-6">
            ...
            <div className="space-y-4">
              ...
              {productFileName || initialData?.fileUrl ? (
                <div className="p-3 rounded-xl bg-surface border border-card-border flex items-center justify-between group animate-fade-in mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col">
                      <span className="text-sm font-medium text-foreground truncate max-w-[200px]">
                        {productFileName || "Current Uploaded File"}
                      </span>
                  ...
                  <Button
                    ...
                    onClick={() => {
                      removeProductFile();
                      if (initialData) initialData.fileUrl = "";
                    }}
